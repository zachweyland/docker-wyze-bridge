    ...
    def setup_streams(self):
        ...
        for cam in self.api.filtered_cams():
            ...
            options = WyzeStreamOptions(
                ...
                reconnect=(not ON_DEMAND) or is_livestream(cam.name_uri) or (cam.is_kvs and self.gst_rtsp.enabled),
            )
            ...
            # Add substream alias for Scrypted (and other consumers) that expect /path-sub path
            # For KVS cameras, this will proxy the main stream since no native substream exists
            if not (env_bool(f"SUBSTREAM_{cam.name_uri}") or (env_bool("SUBSTREAM") and cam.can_substream)):
                # No native substream - add alias for Scrypted compatibility
                self.mtx.add_substream_alias(cam.name_uri)
